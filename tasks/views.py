...
def addperson(request, event_id):
    ...
    for tag in event.tag_set.all():
        if str(tag.id) in selected_tags:
            person.tags.add(tag)
    person.save()
    return render(request, 'tasks/event.html', {'event': event, 'error_message': "Person added successfully"})

def find_person_for_task(event, persons, task):
    sortedlist = sorted(persons, key=attrgetter('task_count'))
    print("Person list:", sortedlist, 'for task', task)
    assignee = None
    for person in sortedlist:
        person_valid = True
        if task in person.tasks:
            print(person, 'already has', task)
            break
        ...
        if person_valid:
            assignee = person
            print("Accepted person", person, task)
            break
